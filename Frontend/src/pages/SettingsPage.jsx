    ...
    >
        ...
        <Box sx={{flex: 1, overflow: 'hidden'}}>
            ...
            <Typography variant="body1"
                        sx={{fontWeight: 700, color: '#1e293b', mt: 0.5, wordBreak: 'break-word', lineHeight: 1.4}}>
...
const SettingsPage = () => {
    ...
    // Vì regProfile gắn liền form register vào input, hàm onChange gốc sẽ bị đè
    // -> dùng watchProfile để lấy giá trị real-time.
    const {
        register: regProfile,
        ...
        switch (activeMenu) {
                ...
                return (
                    <Box component={motion.div} initial={{opacity: 0, x: 20}} animate={{opacity: 1, x: 0}}
                        ...
                        <Grid container spacing={2.5}>
                            <Grid item xs={12} sm={6}><BentoCard icon={<BadgeOutlined/>} label="Tên đăng nhập"
                                                                 value={profileData?.username}/></Grid>
                            <Grid item xs={12} sm={6}><BentoCard icon={<Person/>} label="Họ và tên"
                                                                 value={profileData?.fullName}/></Grid>
                            <Grid item xs={12} sm={6}><BentoCard icon={<EmailOutlined/>} label="Email"
                                                                 value={profileData?.email}/></Grid>
                            <Grid item xs={12} sm={6}><BentoCard icon={<PhoneIphoneOutlined/>} label="Số điện thoại"
                                                                 value={profileData?.phoneNumber}/></Grid>
                        </Grid>
                        ...
                        {profileData?.role?.includes("STUDENT") && (
                            <>
                                <Divider sx={{my: 4, '&::before, &::after': {borderColor: 'rgba(0,0,0,0.08)'}}}>
                                    <Chip label="Thông tin Học vấn & Liên hệ" sx={{
                                        fontWeight: 700,
                                        color: '#475569',
                                        bgcolor: '#f1f5f9',
                                        letterSpacing: '0.5px'
                                ...
                                <Grid container spacing={2.5}>
                                    <Grid item xs={12} sm={6} md={4}><BentoCard icon={<BadgeOutlined/>}
                                                                                label="Mã sinh viên"
                                                                                value={profileData?.student?.studentCode}/></Grid>
                                    <Grid item xs={12} sm={6} md={4}><BentoCard icon={<SchoolOutlined/>}
                                                                                label="Ngành học"
                                                                                value={profileData?.student?.major}/></Grid>
                                    <Grid item xs={12} sm={6} md={4}><BentoCard icon={<ClassOutlined/>} label="Lớp"
                                                                                value={profileData?.student?.classRoom}/></Grid>
                                    <Grid item xs={12} sm={6} md={4}><BentoCard icon={<CakeOutlined/>} label="Ngày sinh"
                                                                                value={profileData?.student?.dateOfBirth}/></Grid>
                                    <Grid item xs={12} md={8}><BentoCard icon={<LocationOnOutlined/>} label="Địa chỉ"
                                                                         value={profileData?.student?.address}/></Grid>
                                </Grid>
                            </>
                        )}
                        ...
                        {profileData?.role?.includes("MENTOR") && (
                            <>
                                <Divider sx={{my: 4, '&::before, &::after': {borderColor: 'rgba(0,0,0,0.08)'}}}>
                                    <Chip label="Thông tin Chuyên môn" sx={{
                                        fontWeight: 700,
                                        color: '#475569',
                                        bgcolor: '#f1f5f9',
                                        letterSpacing: '0.5px'
                                ...
                                <Grid container spacing={2.5}>
                                    <Grid item xs={12} sm={6}><BentoCard icon={<BusinessOutlined/>}
                                                                         label="Khoa / Phòng ban"
                                                                         value={profileData?.mentor?.department}/></Grid>
                                    <Grid item xs={12} sm={6}><BentoCard icon={<StarBorder/>} label="Học hàm / Học vị"
                                                                         value={profileData?.mentor?.academicRank}/></Grid>
                                </Grid>
                            </>
                        )}
                    </Box>
            ...
            case 'edit':
                return (
                    <Box component={motion.div} initial={{opacity: 0, x: 20}} animate={{opacity: 1, x: 0}}
                         exit={{opacity: 0, x: -20}}>
                        <Box sx={{mb: 4}}><Typography variant="h6" sx={{fontWeight: 800, color: '#1e293b'}}>Chỉnh Sửa
                            Thông Tin</Typography></Box>
                        <Box component="form" onSubmit={handleProfileSubmit(onUpdateProfile)}>
                            <Stack spacing={3}>
                                <TextField fullWidth label="Tên đăng nhập" {...regProfile("username")} disabled
                                           sx={{'& .MuiOutlinedInput-root': {bgcolor: '#f8fafc', borderRadius: 3}}}/>
                                <TextField fullWidth
                                           label="Họ và Tên" {...regProfile("fullName", {required: "Bắt buộc"})}
                                ...
                                {profileData?.role?.includes("STUDENT") && (
                                    <>
                                        ...
                                        <TextField fullWidth label="Mã sinh viên" {...regProfile("studentCode")}
                                                   disabled/>
                                        <TextField fullWidth
                                                   label="Ngành học (*)" {...regProfile("major", {required: "Vui lòng nhập ngành học"})}
                                                   error={!!profileErrors.major}
                                                   helperText={profileErrors.major?.message}/>
                                        <TextField fullWidth
                                                   label="Lớp (*)" {...regProfile("classRoom", {required: "Vui lòng nhập lớp"})}
                                        ...
                                        {/* -- TRICK UI CHUYỂN ĐỔI NGÀY THÁNG DD/MM/YYYY -- */}
                                        <TextField
                                            fullWidth
                                            label="Ngày sinh"
                                            type={isDobFocused ? "date" : "text"}
                                            // Sử dụng watchProfile("dateOfBirth") để kéo giá trị yyyy-MM-dd đang lưu ngầm
                                            value={isDobFocused ? (watchProfile("dateOfBirth") || "") : getDisplayDate(watchProfile("dateOfBirth"))}
                                            onChange={(e) => setProfileValue("dateOfBirth", e.target.value)}
                                            onFocus={() => setIsDobFocused(true)}
                                            onBlur={() => setIsDobFocused(false)}
                                            InputLabelProps={{shrink: isDobFocused || !!watchProfile("dateOfBirth")}}
                                            placeholder="dd/MM/yyyy"
                                            sx={{'& .MuiOutlinedInput-root': {bgcolor: '#ffffff', borderRadius: 3}}}
                                        />
                                    </>
                                )}
                                ...
                                {profileData?.role?.includes("MENTOR") && (
                                    <>
                                        ...
                                        <TextField fullWidth
                                                   label="Khoa / Phòng ban (*)" {...regProfile("department", {required: "Vui lòng nhập khoa"})}
            ...
            case 'security':
                return (
                    <Box component={motion.div} initial={{opacity: 0, x: 20}} animate={{opacity: 1, x: 0}}
                         exit={{opacity: 0, x: -20}}>
                        <Box sx={{mb: 4}}><Typography variant="h6" sx={{fontWeight: 800, color: '#1e293b'}}>Bảo Mật Tài
                            Khoản</Typography></Box>
                        <Box component="form" onSubmit={handlePasswordSubmit(onChangePassword)}>
                            <Stack spacing={3}>
                                <FormControl fullWidth variant="outlined" error={!!passwordErrors.oldPassword}>
                                    ...
                                    <OutlinedInput
                                        type={showOldPassword ? "text" : "password"} {...regPassword("oldPassword", {required: "Bắt buộc"})}
                                ...
                                <FormControl fullWidth variant="outlined" error={!!passwordErrors.newPassword}>
                                    ...
                                    <OutlinedInput
                                        type={showNewPassword ? "text" : "password"} {...regPassword("newPassword", {required: "Bắt buộc"})}
                                        endAdornment={<InputAdornment position="end"><IconButton
                                            onClick={() => setShowNewPassword(!showNewPassword)}
                                            edge="end">{showNewPassword ? <VisibilityOff/> : <Visibility/>}</IconButton></InputAdornment>}
                                ...
                                <FormControl fullWidth variant="outlined" error={!!passwordErrors.confirmPassword}>
                                    ...
                                    <OutlinedInput
                                        type={showConfirmPassword ? "text" : "password"} {...regPassword("confirmPassword", {validate: (val) => val === watch("newPassword") || "Mật khẩu không khớp"})}
                                        endAdornment={<InputAdornment position="end"><IconButton
                                            onClick={() => setShowConfirmPassword(!showConfirmPassword)}
                                            edge="end">{showConfirmPassword ? <VisibilityOff/> :
                                            <Visibility/>}</IconButton></InputAdornment>} label="Xác nhận mật khẩu"
                    ...
                    <Box sx={{
                        ...
                        <Box sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 5}}>
                            <Box sx={{position: "relative"}}>
                                <input accept="image/*" style={{display: "none"}} id="avatar-upload-input" type="file"
                                        ...
                                        badgeContent={
                                            <motion.div whileHover={{scale: 1.1, rotate: 15}} whileTap={{scale: 0.9}}>
                                                <IconButton component="span" sx={{
                                                    ...
                                                    {loadingAvatar ? <CircularProgress size={20} color="inherit"/> :
                            ...
                            <Typography variant="h6"
                                        sx={{fontWeight: 800, mt: 2, color: '#1e293b', textAlign: 'center'}}>
                                {profileData?.fullName || profileData?.username}
                            </Typography>
                            <Chip icon={<VerifiedUserOutlined sx={{fontSize: 16}}/>}
                                  label={profileData?.role?.replace('ROLE_', '') || 'USER'} size="small" sx={{
                                mt: 1,
                        ...
                        <Stack spacing={1} sx={{flexGrow: 1}}>
                            <SidebarButton active={activeMenu === 'profile'} onClick={() => setActiveMenu('profile')}
                                           startIcon={<Person/>}>Thông tin chung</SidebarButton>
                            <SidebarButton active={activeMenu === 'edit'} onClick={() => setActiveMenu('edit')}
                                           startIcon={<Edit/>}>Cập nhật hồ sơ</SidebarButton>
                            <SidebarButton active={activeMenu === 'security'} onClick={() => setActiveMenu('security')}
                                           startIcon={<Security/>}>Bảo mật</SidebarButton>
                        </Stack>
                    </Box>
