...
const AssessmentRoundsManagement = () => {
    ...
    // 4. HÀM TẢI DANH SÁCH PHASE
    const fetchAllPhases = async () => {
        try {
            const res = await internshipPhaseApi.getAllPhases("", 0, 100);
            setAvailablePhases(res?.content || []);
        } catch (err) {
    ...
    const handleOpenModal = (round = null) => {
        // Gọi tải danh sách phase khi mở modal
        fetchAllPhases();
                            ...
                            >
                                        ...
                                        <Box>
                                            ...
                                            <Chip
                                                ...
                                                sx={{
                                                    ...
                                                    fontSize: '0.7rem',
                                                    height: 22,
                                                    bgcolor: round.isDeleted ? 'rgba(211, 47, 47, 0.1)' : 'rgba(46, 125, 50, 0.1)',
                                                    color: round.isDeleted ? '#d32f2f' : '#2e7d32'
                                                }}
                                            />
                                        </Box>
                                    </Box>
                                ...
                                <Stack spacing={1.5} sx={{
                                    ...
                                    <Stack direction="row" alignItems="center" gap={1.5}>
                                        ...
                                        <Typography variant="body2" sx={{fontWeight: 600}}>Giai đoạn: <span style={{
                                            fontWeight: 400,
                                            color: '#1976d2'
                                        }}>{round.phaseName || "N/A"}</span></Typography>
                                    </Stack>
                                    <Stack direction="row" alignItems="center" gap={1.5}>
                                        <EventAvailableIcon sx={{color: '#757575', fontSize: 18}}/>
                                        <Typography variant="body2" sx={{fontWeight: 600}}>Thời gian: <span
                                            style={{fontWeight: 400}}>{round.startDate || "..."} đến {round.endDate || "..."}</span></Typography>
                                    </Stack>
                                </Stack>
                                ...
                                <Stack direction="row" justifyContent="space-between"
                                    ...
                                    <Button startIcon={<VisibilityIcon/>} size="small" variant="contained" color="info"
                                            onClick={() => navigate(`/admin/assessment-rounds/${round.id}`)}
                                            sx={{borderRadius: 2, fontWeight: 600, boxShadow: 0}}>
                                        Chi tiết
                                    </Button>
                            ...
                            <Paper sx={{
                                ...
                                <Box sx={{
                                    ...
                                    <Typography variant="h5" sx={{
                                        fontWeight: 800,
                                        color: '#1a237e'
                                    }}>{editingRound ? "Cập nhật Vòng Đánh giá" : "Tạo Vòng Đánh giá mới"}</Typography>
                                ...
                                <Box sx={{p: 4, bgcolor: '#fff', overflowY: 'auto'}}>
                                    <Stack spacing={3}>
                                        <TextField fullWidth label="Tên vòng đánh giá" value={formData.roundName}
                                                   onChange={(e) => setFormData({
                                                       ...formData,
                                                       roundName: e.target.value
                                                   })}/>
                                        <TextField fullWidth label="Mô tả chi tiết" value={formData.description}
                                                   onChange={(e) => setFormData({
                                                       ...formData,
                                        ...
                                        <FormControl fullWidth>
                                            <InputLabel id="phase-select-label">Giai đoạn thực tập (Phase)</InputLabel>
                                            <Select
                                                labelId="phase-select-label"
                                                label="Giai đoạn thực tập (Phase)"
                                                value={formData.phaseId}
                                                onChange={(e) => setFormData({...formData, phaseId: e.target.value})}
                                            >
                                                {availablePhases.length === 0 ? (
                                                    <MenuItem disabled value="">
                                                        <em>Chưa có giai đoạn nào</em>
                                                    </MenuItem>
                                                ) : (
                                        ...
                                        <Box sx={{
                                            ...
                                            <Typography variant="subtitle2"
                                                        sx={{fontWeight: 600, color: "#555", mb: 1}}>Chọn Tiêu chí &
                                                Nhập Trọng số</Typography>
                                            <Autocomplete
                                                multiple options={allCriteria} getOptionLabel={(o) => o.criterionName}
                                                    ...
                                                    {formData.roundCriteria.map((item, index) => (
                                                        <Box key={item.criterionId} sx={{
                                                            ...
                                                            <Typography variant="body2" sx={{
                                                                ...
                                                                flex: 1,
                                                                pr: 2
                                                            }}>{item.criterionName}</Typography>
                                                            <TextField
                                                                type="number" size="small" placeholder="Trọng số (%)"
                                                                sx={{width: 100}} value={item.weight ?? ""}
                                                                onChange={(e) => {
                                        ...
                                        <Box sx={{
                                            ...
                                            <Typography variant="body2" sx={{fontWeight: 600, color: "#555"}}>Trạng thái
                                                hoạt động</Typography>
                                            <FormControlLabel
                                                control={<Switch checked={!formData.isDeleted}
                                                                 onChange={(e) => setFormData({
                                                                     ...formData,
                                                ...
                                                }}>{!formData.isDeleted ? "Hoạt động" : "Khóa"}</Typography>}
                                ...
                                <Box sx={{
                                    ...
                                    <Button fullWidth variant="outlined" color="inherit" onClick={handleCloseModal}
                                            sx={{borderRadius: 2, py: 1.5}}>Hủy bỏ</Button>
                        ...
                        >
                            <Paper sx={{
                                ...
                                <Stack alignItems="center" spacing={2} sx={{textAlignment: 'center', mb: 3}}>
                                    ...
                                    <Typography variant="h6" sx={{fontWeight: 800, color: '#1a237e'}}>Xác nhận
                                        xóa?</Typography>
                                    <Typography variant="body2" color="text.secondary">
                                        Bạn có chắc chắn muốn xóa vòng đánh
                                        giá <strong>{roundToDelete?.roundName}</strong>? Hành động này không thể hoàn
                                        tác.
                                    </Typography>
                                </Stack>
                                <Stack direction="row" spacing={2}>
                                    ...
                                    <Button fullWidth variant="contained" color="error" onClick={handleConfirmDelete}
                                            sx={{borderRadius: 2, py: 1.2, fontWeight: 600}}>Xóa ngay</Button>
                                </Stack>
                            </Paper>
                        </motion.div>
                    )}
