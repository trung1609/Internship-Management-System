  ...
  return (
    <Box sx={{ p: 4, minHeight: '100vh', backgroundColor: '#f4f6f8' }}>
            ...
            >
              ...
              >
                <Box sx={{ 
                  ...
                  background: 'rgba(26, 35, 126, 0.03)', zIndex: 0 
                }} />

                <Stack direction="row" spacing={2} alignItems="center" sx={{ position: 'relative', zIndex: 1, mb: 2 }}>
                  <Avatar sx={{ width: 56, height: 56, bgcolor: getRoleColor(user.role) + '.main', fontWeight: 'bold' }}>
                ...
                <Stack spacing={1.5} sx={{ position: 'relative', zIndex: 1, mb: 3 }}>
                  ...
                  <Box>
                    <Chip label={getRoleLabel(user.role)} color={getRoleColor(user.role)} size="small" sx={{ fontWeight: 'bold' }} />
                  </Box>
                </Stack>
      ...
      <Modal 
        ...
        sx={{ display: 'flex', alignItems: 'center', justifyOntext: 'center', justifyContent: 'center', backdropFilter: 'blur(3px)' }}
      >
        <AnimatePresence>
          {openModal && (
            <motion.div
            ...
            >
              <Paper sx={{ borderRadius: 4, overflow: 'hidden', boxShadow: '0 24px 48px rgba(0,0,0,0.25)' }}>
                ...
                <Box sx={{ p: 4, bgcolor: '#fff' }}>
                  <Stack spacing={3}>
                    <TextField fullWidth label="Tên đăng nhập" value={formData.username} onChange={(e) => setFormData({ ...formData, username: e.target.value })} disabled={editingUser !== null} />
                    <TextField fullWidth label="Email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
                    <TextField fullWidth label="Họ và tên" value={formData.fullName} onChange={(e) => setFormData({ ...formData, fullName: e.target.value })} />
                    {!editingUser && (
                      <TextField fullWidth label="Mật khẩu" type="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} />
                    )}
                    <TextField fullWidth label="Số điện thoại" value={formData.phoneNumber} onChange={(e) => setFormData({ ...formData, phoneNumber: e.target.value })} />
                ...
                <Box sx={{ p: 3, pt: 0, display: 'flex', gap: 2, bgcolor: '#fff' }}>
                  <Button fullWidth variant="outlined" color="inherit" onClick={handleCloseModal} sx={{ borderRadius: 2, py: 1.5 }}>
                    Hủy bỏ
                  </Button>
                  <Button fullWidth variant="contained" onClick={handleSave} sx={{ borderRadius: 2, py: 1.5, boxShadow: '0 8px 16px rgba(25, 118, 210, 0.2)' }}>
                    Lưu thông tin
                  </Button>
                </Box>
              </Paper>
            ...
            >
              <Paper sx={{ borderRadius: 4, overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.3)', p: 3, bgcolor: '#fff' }}>
                <Stack alignItems="center" spacing={2} sx={{ textAlignment: 'center', textAlign: 'center', mb: 3 }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Bạn có chắc chắn muốn xóa tài khoản <strong>{userToDelete?.fullName}</strong> (@{userToDelete?.username})? Hành động này không thể hoàn tác.
                  </Typography>
                </Stack>
