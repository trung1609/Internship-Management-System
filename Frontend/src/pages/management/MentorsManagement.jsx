...
import {mentorApi, userApi} from "../../api/resourceApi";
...
const MentorsManagement = () => {
    ...
    const fetchAvailableUsers = async () => {
        try {
            const response = await userApi.getAvailableMentors();
            setAvailableUsers(response || []);
        } catch (err) {
            console.error("Lỗi khi tải danh sách user khả dụng:", err);
        }
    };
                        ...
                        >
                            ...
                            >
                                ...
                                <Stack direction="row" spacing={2} alignItems="center"
                                    ...
                                    <Avatar src={mentor.avatarUrl}
                                            sx={{width: 56, height: 56, bgcolor: 'warning.main', fontWeight: 'bold'}}>
                                        {!mentor.avatarUrl && mentor.fullName ? mentor.fullName.charAt(0) :
                                ...
                                <Stack direction="row" spacing={1}
                                       sx={{mb: 2, position: 'relative', zIndex: 1, flexWrap: 'wrap', gap: 1}}>
                                    {mentor.department && <Chip label={mentor.department} size="small" color="primary"
                                                                variant="outlined"/>}
                                    {mentor.academicRank &&
                                        <Chip label={mentor.academicRank} size="small" color="warning"
                                              variant="outlined"/>}
                                </Stack>
                                ...
                                <Stack spacing={1} sx={{position: 'relative', zIndex: 1, mb: 3, flexGrow: 1}}>
                                    ...
                                    <Typography variant="body2"><strong>SĐT:</strong> {mentor.phoneNumber || 'N/A'}
                                    </Typography>
                                    <Typography
                                        variant="body2"><strong>Khoa/Phòng:</strong> {mentor.department || 'N/A'}
                                    </Typography>
                                    <Typography variant="body2"><strong>Học
                                        hàm/vị:</strong> {mentor.academicRank || 'N/A'}</Typography>
                                </Stack>
                            ...
                            <Paper sx={{
                                ...
                                <Box sx={{p: 4, bgcolor: '#fff', overflowY: 'auto'}}>
                                    <Stack spacing={3}>
                                        ...
                                        {/* --- DROPDOWN CHỌN USER THAY VÌ NHẬP TAY --- */}
                                        {!editingMentor && (
                                            <FormControl fullWidth>
                                                <InputLabel id="select-mentor-user-label">Chọn tài khoản liên kết
                                                    (User)</InputLabel>
                                                <Select
                                                    labelId="select-mentor-user-label"
                                                    label="Chọn tài khoản liên kết (User)"
                                                    value={formData.userId}
                                                    onChange={(e) => setFormData({...formData, userId: e.target.value})}
                                                >
                                                    {availableUsers.length === 0 ? (
                                                        <MenuItem disabled value="">
                                                            <em>Không có tài khoản cố vấn nào khả dụng</em>
                                                        </MenuItem>
                                                    ) : (
                                                        availableUsers.map((u) => (
                                                            <MenuItem key={u.userId} value={u.userId}>
                                                                {u.fullName} - @{u.username} (ID: {u.userId})
                                                            </MenuItem>
                                                        ))
                                                    )}
                                                </Select>
                        ...
                        >
                            <Paper sx={{
                                ...
                                <Stack alignItems="center" spacing={2}
                                    ...
                                    <Typography variant="body2" color="text.secondary">
                                        Bạn có chắc chắn muốn xóa cố
                                        vấn <strong>{mentorToDelete?.fullName || 'này'}</strong>? Hành động này không
                                        thể hoàn tác.
                                    </Typography>
                                </Stack>
